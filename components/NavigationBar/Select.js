...
export class Select extends Component {
    ...
    getItems = () => {
        ...
        return items.reduce((acc, item) => {
            if (searchText === "" || item.label.toLowerCase().indexOf(searchText.toLowerCase()) !== -1) {
                acc.push(
                    <div key={item.value} onClick={() => this.handleChange(item.value)}
    ...
    render() {
        const { items, selectedItem, showSearch, showHeader } = this.props;
            ...
            <Popover
                ...
                content={(
                    <Fragment>
                        ...
                        {showSearch && <div className={classNames(`${clsPrefix}-search`)}>
                            <Input
            ...
            >
                <div className={classNames(`${clsPrefix}-selection`, { open: !!visible })}>
                    <div className={(`${clsPrefix}-selected-value`)}>
                        {showSelectedIcon && <div className={(`${clsPrefix}-selected-icon`)}>
                            {selectedItemLabel[0]}
                        </div>}
